...
def create_products():
    ...
    # Uncomment this line once READ is implemented
    # location_url = url_for("get_product", product_id=product.id, _external=True)
    location_url = "/"  # Temporary location until get_product is ready
    return jsonify(message), status.HTTP_201_CREATED, {"Location": location_url}


######################################################################
# L I S T   A L L   P R O D U C T S   (with filtering support)
######################################################################
@app.route("/products", methods=["GET"])
def list_products():
    """Returns all Products or filters them by query parameters."""
    ...
    # Filter by availability (accept true/false strings)
    if available is not None:
        ...
        if available.lower() in ["true", "1", "yes"]:
            ...
            abort(
                ...
                "Invalid value for 'available' query parameter. Use true or false.",
            )
...
def update_product(product_id):
    ...
    Expects JSON data and returns the updated product.
    """
    check_content_type("application/json")
    ...
    if "name" not in data or not data["name"]:
        abort(status.HTTP_400_BAD_REQUEST, "Product 'name' is required and cannot be empty")
